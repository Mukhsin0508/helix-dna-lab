...
import { featureKey, featureLabel, formatScore, matchingRows, rankedRows, rowLabel, scoreValue, trackGroups, unitLabel } from '../analysisUtils';
...
function WrapText({ text, x, y, width, fontSize = 10, fill = INK }: { text: string; x: number; y: number; width: number; fontSize?: number; fill?: string }) {
  ...
  for (const word of words) { if ((line + ' ' + word).length * fontSize * .54 > width && line) { lines.push(line); line = word; } else line += (line ? ' ' : '') + word; } if (line) lines.push(line);
...
export default function AnalysisFigure({ dataset, settings, svgRef, onSelect }: Props) {
  ...
  if (dataset.kind === 'tracks') {
    const groups = trackGroups(dataset.rows).filter(group => !settings.track || group.key === settings.track).slice(0, settings.limit);
    const panelHeight = mobile ? 195 : 180; const height = Math.max(330, groups.length * panelHeight + 85);
    return <div ref={wrapper}><svg ref={svgRef} className="plot-svg" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${settings.title}. Reference and alternate tracks. Zero-based genomic positions.`}>
      ...
      {groups.map((group, index) => {
        const left = mobile ? 49 : 75, right = width - 32, top = index * panelHeight + 50, bottom = top + 95;
        const minPos = group.rows[0].position, maxPos = group.rows[group.rows.length - 1].position;
        ...
        return <g key={group.key}>
          <text x={left} y={top - 19} fontSize={11} fill={INK} fontFamily={MONO}>{group.label}</text>
          ...
          {(['reference', 'alternate'] as const).map(kind => <g key={kind}><path d={group.rows.map((row, i) => `${i ? 'L' : 'M'} ${px(row.position)} ${py(row[kind])}`).join(' ')} fill="none" stroke={kind === 'reference' ? BLUE : CORAL} strokeWidth={1.8} />{group.rows.length < 40 && group.rows.map(row => <circle key={row.position} cx={px(row.position)} cy={py(row[kind])} r={2.4} fill={kind === 'reference' ? BLUE : CORAL}><title>{`${group.label} · position ${row.position} (0-based) · ${kind}: ${row[kind]}`}</title></circle>)}</g>)}
